package net.sockmc.socky;

import net.minecraft.util.Identifier;

public class Ids {
    public static final String MOD_ID = "socky";

    public static final Identifier SOCKY = new Identifier(MOD_ID, "socky");
    public static final Identifier SOCKY_BALE = new Identifier(MOD_ID, "socky_bale");
    public static final Identifier SOCKY_BODY = new Identifier(MOD_ID, "socky_body");
    public static final Identifier SOCKY_CAPE = new Identifier(MOD_ID,"socky_cape");
    public static final Identifier SOCKY_PANTS = new Identifier(MOD_ID,"socky_pants");
    public static final Identifier SOCKY_SCARF = new Identifier(MOD_ID,"socky_scarf");
}
